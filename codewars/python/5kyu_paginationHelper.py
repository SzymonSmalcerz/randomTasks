...
class PaginationHelper:
  ...
  def page_count(self):
      return math.ceil(self.item_count()/float(self.items_per_page))
  ...
  def page_index(self,item_index):
      if item_index >= self.item_count() or item_index < 0: 
          return -1
      elif (item_index+1) <= self.items_per_page * (self.page_count()-1):
          return math.ceil((item_index+1)/float(self.items_per_page))-1
      else:
