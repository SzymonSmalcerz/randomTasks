/*
	...
	its digits is divisible by 2, reverse that chunk; otherwise 
...
function revrot(str, sz) {
    ...
    while(index <= str.length){
      let temp = str.slice(index-sz,index).split("");
      let value = temp.map(val => parseInt(val)).reduce((total,val) =>  Math.pow(val,3) + total,0);
      result += value%2 ? temp.slice(1).join("") + temp[0] : temp.reverse().join("");
      index += sz;
    }
    return result;
}
