...
function peacefulYard(yard, minDistance) {
    var catsCoords = { //x : xPos, y : yPos
      'L' : {},
      'R' : {},
      'M' : {}
    } 
    
    yard.forEach((row,xIndex,arr) => {
      var tempRow = row.split("")
      checkForCats(tempRow,xIndex,catsCoords)
    })
    
    return canCatsSleepPeacefully(catsCoords,minDistance)
}

function checkForCats(row,rowIndex,catsCoords){
  
  for (var property in catsCoords) {
    if (catsCoords.hasOwnProperty(property)) {
        var columnIndex = row.indexOf(property)
        if(columnIndex != -1){
          catsCoords[property] = {
                                    x : rowIndex,
                                    y : columnIndex
...
function canCatsSleepPeacefully(catsCoords,minDistance){
  
  var tableOfCoords = []
  
  //extract coords to table for readability
  for (var property in catsCoords) {
    if (catsCoords.hasOwnProperty(property)) {
        if(catsCoords[property]){
          tableOfCoords.push(catsCoords[property])
        }
    }
  }
  return !(isThereCollision(tableOfCoords,minDistance))

}


function isThereCollision(tableOfCoords,minDistance){
  for(let i=0;i<tableOfCoords.length;i++){
    for(let j=0;j<tableOfCoords.length;j++){
      if(i==j){continue}
      if(Math.pow((tableOfCoords[i].x - tableOfCoords[j].x),2) + Math.pow((tableOfCoords[i].y - tableOfCoords[j].y),2) < Math.pow(minDistance,2)){
        return true;
      }
    }
  }
