/*
	...
	race has a certain worth when battling against others. On the side of good 
...
function goodVsEvil(good, evil){
  ...
  let goodForcesValues = [1,2,3,3,4,10]
  let g = getCombinedValue(good,goodForcesValues); //goodForcesCombinedValue
  
  let evilForcesValues = [1,2,2,2,3,5,10]
  let e = getCombinedValue(evil,evilForcesValues); //evilForcesCombinedValue
...
function getCombinedValue(arr,values){
  
  return arr.split(" ").map(val => parseInt(val)).reduce((total,val,index) => total + val * values[index] ,0)
