...
function getRequired(player, enemy) {
  ...
  let playerPts = player.reduce((t,v) => t+v,0);
  let enemyPts = enemy.reduce((t,v) => t+v,0);
  

  if(enemyPts - 6 >= playerPts){return "Auto-lose"}
  else if(playerPts - 6 >= enemyPts){return "Auto-win"}
  else if(enemyPts - 5 == playerPts){return "Pray for a tie!"}
  else if(enemyPts == playerPts){return "Random"}
  else if(enemyPts < playerPts){
    let a = 6 - (playerPts-enemyPts-1);
    let b = 6;
    return `(${a}..${b})`;
  } else{
    let a = 6 - (enemyPts-playerPts+1);
    let b = 1;
    return `(${b}..${a})`;
  }
}
