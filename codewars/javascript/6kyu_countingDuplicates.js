...
function duplicateCount(text){
  ...
  text.split("").map(val => val.toUpperCase()).forEach(val => duplicates[val] ? duplicates[val] += 1 : duplicates[val] = 1);
  
  let result = 0;
  for (var letter in duplicates) {
    if (duplicates.hasOwnProperty(letter)) {
        if(duplicates[letter] > 1){result += 1;}
    }
  }
  return result;
}
