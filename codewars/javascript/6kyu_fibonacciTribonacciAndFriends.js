/*
	...
	Well, time to expand the family a little more: think of a Quadribonacci starting 
	with a signature of 4 elements and each following element is the sum of the 4 previous, 
	...
	Well, guess what? You have to build a Xbonacci function that takes a signature of 
	X elements - and remember each next element is the sum of the last X elements 
...
function Xbonacci(signature,n){
  ...
  if(n <= 0 || signature.join("").search(/[^0-9.]/) != -1 || x == 0){
    return []
  } else if(n<=x){
    return signature.slice(0,n)
  } else {
    n-=x;
    let resArray = signature.slice();
    while(n>0){
      let newValue = 0;
      for(let i=0;i<x;i++){
