/*
	...
	You can also directly jump from from the initial cell(first cell) past the end of the array 
	if they are integers to the right of that cell.
...
function canJump(array){
  let index = 0;
  let maxIndex = 0;
  while(index < array.length-1 && maxIndex < array.length){
    if(index > maxIndex){
      return false;
    }
    maxIndex = Math.max(maxIndex,index + array[index]);
    index+=1;
  }
  return index < array.length-1 || maxIndex > array.length - 1;
}
