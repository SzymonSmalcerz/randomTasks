/*
  ...
  in some Arcade back in the 90s or emulating it nowadays with the numerous emulators for retro consoles.
  ...
  wether successful or not, see tests);
  Rules
...
function streetFighterSelection(fighters, p, moves) {
  let result = [];
  moves.forEach(move => {
    p = functions[move](p);
    result.push(fighters[p[1]][p[0]]);
  });
  return result;
}
...
var functions = {
  ...
  "left" : function(position) {
    return [(position[0] + 5)%6,position[1]];
  },
  "right" : function(position) {
    return [(position[0] + 1)%6, position[1]];
  }
}
