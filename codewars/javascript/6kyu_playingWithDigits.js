...
function digPow(n, p){
  var sum = getSumOfNDigitsToP(n,p);
  return sum % n == 0 ? sum/n : -1;
}

var getSumOfNDigitsToP = (n,p) => {
  return n.toString().split("").map((val,index) => Math.pow(parseInt(val),(p+index))).reduce((total,val) => val + total,0);
}
