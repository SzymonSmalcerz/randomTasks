/*
  ...
  letter of the book category. In the bookseller's stocklist each
  code c is followed by a space and by a positive integer n (int n >= 0)
...
function stockList(listOfArt, listOfCat){
  if(!listOfArt.length || !listOfCat){return ""};
  let dictionary = {};
  listOfArt.forEach(v => {
    dictionary[v[0]] = dictionary[v[0]] || 0;
    dictionary[v[0]] += ~~(v.split(' ')[1]);
  });

  return listOfCat.map(v => `(${v} : ${dictionary[v] || 0})`).join(' - ');
}
